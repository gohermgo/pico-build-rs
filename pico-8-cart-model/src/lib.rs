...
impl<'a> Section<'a> {
    fn new_inner(r#type: SectionType, line_number: usize, data: Cow<'_, [u8]>) -> Section<'_> {
        match r#type {
            SectionType::Lua => Section::Lua { line_number, data },
            SectionType::Gfx => Section::Gfx { line_number, data },
            SectionType::Gff => Section::Gff { line_number, data },
            SectionType::Label => Section::Label { line_number, data },
            SectionType::Map => Section::Map { line_number, data },
            SectionType::Sfx => Section::Sfx { line_number, data },
            SectionType::Music => Section::Music { line_number, data },
        }
    }
    ...
    fn unwrap(self) -> (SectionType, usize, Cow<'a, [u8]>) {
        ...
        match self {
            Section::Lua { data, line_number }
            | Section::Gfx { data, line_number }
            | Section::Gff { data, line_number }
            | Section::Label { data, line_number }
            | Section::Map { data, line_number }
            | Section::Sfx { data, line_number }
            | Section::Music { data, line_number } => (r#type, line_number, data),
        }
    }
